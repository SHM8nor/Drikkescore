...
import { prepareWeeklyConsumptionChartData } from '../../utils/analyticsChartHelpers';
...
export default function WeeklyStatsChart({ weeklyData }: WeeklyStatsChartProps) {
  ...
  if (weeklyData.length === 0) {
    return (
      ...
      >
        Ingen ukentlige data tilgjengelig
      </Box>
    );
  }

  // Extract data for MUI BarChart
  const xAxisData = chartData.map((d) => d.week);
      ...
      >
        <Box sx={{ textAlign: 'center' }}>
          ...
          <Typography variant="body2" sx={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>
            {unit === 'beers'
              ? `${weeklyData.reduce((sum, w) => sum + w.beers, 0).toFixed(1)} enheter`
              : `${weeklyData.reduce((sum, w) => sum + w.grams, 0).toFixed(0)}g`}
          </Typography>
        </Box>
        <Box sx={{ textAlign: 'center' }}>
          <Typography variant="caption" sx={{ color: 'var(--color-text-muted)', display: 'block' }}>
          ...
          <Typography variant="body2" sx={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>
            {unit === 'beers'
              ? `${(weeklyData.reduce((sum, w) => sum + w.beers, 0) / weeklyData.length).toFixed(1)} enheter`
              : `${(weeklyData.reduce((sum, w) => sum + w.grams, 0) / weeklyData.length).toFixed(0)}g`}
          </Typography>
        </Box>
      </Box>
    </Box>
